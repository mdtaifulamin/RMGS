...
import { getFormattedDate } from "../EfficiencyAnalysis/util/date"
...
function MachineStatusScanner(){
    ...
    const [locvalue, setlocValue] = useState( null); //updated 31/3/2023
    const [locitems, setlocItems] = useState(locationList.map((e) => ({label:e, value:e})))
    ...
    const [activity, setActivity] = useState(defaultValue?defaultValue.activity:"");
    ...
    useEffect(() => {
        if (defaultValue) {
          setActivity(defaultValue.activity||"")
                ...
                <View style={styles.centeredView}>
                    ...
                    <View style={styles.modalView}>
                        ...
                        <View style={{display:'flex', flexDirection:'column', justifyContent:'flex-start', margin:20}}>
                            ...
                            <View style={{display:'flex', flexDirection:'row', alignItems:'center'}}>
                                ...
                                <DropDownPicker
                                    listMode="MODAL"
                                    open={locopen}
                                    value={locvalue?locvalue:location}
                                    items={locitems}
                                    setOpen={setlocOpen}
                        ...
                        <View style={{ marginVertical:10, padding:10 ,flexDirection:'row'}}>
                            <TouchableOpacity disabled={disabled} style={{backgroundColor:'black', padding:15, borderRadius:20,marginHorizontal:10}} 
                                onPress={() => {
                                            setOpenModal(false); setData("")
                                            }}>
                                <Text style={{color:'white'}} > Cancel </Text> 
                            </TouchableOpacity>
                            <TouchableOpacity disabled={disabled} style={{backgroundColor:'green', padding:15, borderRadius:20,marginHorizontal:10}} onPress={() => {setDisabled(true); sendDataToServer(data)}}>
